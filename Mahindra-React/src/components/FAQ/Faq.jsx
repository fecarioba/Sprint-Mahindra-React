  ...
  return (
    <div className="accordion">
      <button className="accordion-header" onClick={toggleAccordion}>
        <span>{question}</span>
        <i className={`fa-solid fa-chevron-${isOpen ? 'up' : 'down'} arrow ${isOpen ? 'rotate' : ''}`}></i>
      </button>
  ...
  const faqs = [
    {
      ...
      answer: "A Fórmula E é uma categoria de automobilismo que utiliza exclusivamente carros elétricos. É reconhecida pela FIA (Federação Internacional do Automóvel) e é projetada para promover a inovação em veículos elétricos e sustentabilidade no esporte a motor."
    },
    {
      question: "Como funcionam as corridas da Fórmula E?",
      answer: "As corridas da Fórmula E acontecem em circuitos de rua em cidades ao redor do mundo. Cada evento inclui treinos livres, classificação e a corrida propriamente dita, que dura cerca de 45 minutos mais uma volta. A energia dos carros é monitorada de perto e os pilotos precisam trocar de carro durante a corrida devido à limitação da capacidade das baterias."
    },
    {
      question: "Quais são as principais características dos carros da Fórmula E?",
